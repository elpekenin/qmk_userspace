...
static char           qp_log[CONFIG_QP_LOG_N_LINES][CONFIG_QP_LOG_N_CHARS + 1];
static uint8_t        qp_log_current_col;
static char          *qp_log_pointers[CONFIG_QP_LOG_N_LINES];
static deferred_token qp_log_tokens[CONFIG_QP_LOG_N_LINES];
static bool           qp_log_redraw;
static log_level_t    qp_log_levels[CONFIG_QP_LOG_N_LINES];

void sendchar_qp_init(void) {
    memset(qp_log, 0, sizeof(qp_log));
    for (uint8_t i = 0; i < CONFIG_QP_LOG_N_LINES; ++i) {
...
int8_t sendchar_qp(uint8_t chr) {
    if (chr == '\n') {
        ...
        qp_log_pointers[CONFIG_QP_LOG_N_LINES - 1][qp_log_current_col] = 0;

        // Move everything 1 line upwards
        char *temp = qp_log_pointers[0];
        for (uint8_t i = 0; i < CONFIG_QP_LOG_N_LINES - 1; ++i) {
            qp_log_pointers[i] = qp_log_pointers[i + 1];
            qp_log_levels[i]   = qp_log_levels[i + 1];
        }
        qp_log_pointers[CONFIG_QP_LOG_N_LINES - 1] = temp;
        qp_log_levels[CONFIG_QP_LOG_N_LINES - 1]   = LOG_NONE;

        // Reset stuff
        qp_log_current_col                                             = 0;
        qp_log_pointers[CONFIG_QP_LOG_N_LINES - 1][qp_log_current_col] = '\0';
        qp_log_redraw                                                  = true;
    } else if (qp_log_current_col >= CONFIG_QP_LOG_N_CHARS) {
        return 0;
    } else {
        qp_log_pointers[CONFIG_QP_LOG_N_LINES - 1][qp_log_current_col++] = chr;
        qp_log_pointers[CONFIG_QP_LOG_N_LINES - 1][qp_log_current_col]   = '\0';
        qp_log_levels[CONFIG_QP_LOG_N_LINES - 1]                         = get_current_message_level();
        qp_log_redraw                                                    = true;
    }
...
void qp_logging_backend_render(qp_callback_args_t *args) {
    ...
    // Clear space
    qp_rect(args->device, args->x, args->y, qp_get_width(args->device), args->y + (CONFIG_QP_LOG_N_LINES * args->font->line_height), HSV_BLACK, true);
